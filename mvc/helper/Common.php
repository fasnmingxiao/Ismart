...
function num_oder()
    ...
    function echo_menu($array,$parrent_id=0){
        foreach($array as $value){
            if($value['menu_parent_id']== $parrent_id){
                echo "<li><a data-id=".$value['menu_id']." href='".$value['menu_url']."'>".$value['menu_title']."</a>";
                if(array_search($value['menu_id'],array_column($array,'menu_parent_id'))){
                    echo "<ul class='sub-menu'>";
                    echo_menu($array,$value['menu_id']);
                    echo "</ul>";
                }
            }
            echo "</li>";
