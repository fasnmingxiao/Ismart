...
class menuM extends DB{
    ...
    public function update_menu($title, $url, $parent_id, $num_show,$id)
    {
        $qr = "UPDATE `tbl_menu` SET `menu_title`='$title', `menu_url`='$url',`menu_parent_id`= $parent_id, `num_show`=$num_show WHERE `menu_id`= $id";
        $result = false;
        if ($this->db_query($qr, false)) {
            $result = true;
        }
