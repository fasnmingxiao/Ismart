...
//  SEARCH
    $("#s").keyup(function(){
        ...
        if(key== ''){
            ...
            $("#form-search").attr('action', 'http://localhost/Ismart/search&key='+key);
            $.post("http://localhost/Ismart/ajax/keyup_search",{key:key}, function(data){
...
// update cart
    ...
    $.ajax({
        url:"http://localhost/Ismart/ajax/update_cart",
        method:"POST",
        data:{id:id,qty:qty},
        dataType:"JSON",
        success:function(data){
            $("#sub-total-"+id).html(data.sub_total);
            $("#total-price span").html(data.total);
            $("#btn-cart #num").html(data.num_od);
